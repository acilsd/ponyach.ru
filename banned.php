...
if (isset($_POST['appealmessage']) && KU_APPEAL != '') {
	$results = $tc_db->GetAll("SELECT * FROM `".KU_DBPREFIX."banlist` WHERE `type` = '0' AND `ipmd5` = '" . md5($real_ip) . "' AND `id` = " . $tc_db->qstr($_POST['banid']) . "LIMIT 1");
	if (count($results)>0) {
...
$bans_class->BanCheck($real_ip, '', true);
