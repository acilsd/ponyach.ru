...
if (!$singlepost) {
	...
	$board_class->dwoo_data['replythrad'] = $thread;
 	$page .= Haanga::Load( $board_class->board['text_readable'] . '_reply_header.tpl', $board_class->dwoo_data, true);
} else {
	$tpl['title'] = '';
	$tpl['head'] = '';
...
if ($board_class->board['type'] == 1) {
	...
	if ($posts != '0') {
		...
		if(count($relative_to_normal) > 0) {
			$board_class->dwoo_data['posts'] = $relative_to_normal;
			$page .= Haanga::Load('txt_thread.tpl', $board_class->dwoo_data, true);
		}
		...
		if (count($results) > 0){
			...
			$board_class->dwoo_data['posts'] = $results;
			$page .= Haanga::Load('txt_thread.tpl', $board_class->dwoo_data, true);
		}
	}
	...
	$page .= Haanga::Load($board_class->board['text_readable'] . '_thread.tpl', $board_class->dwoo_data, true);
