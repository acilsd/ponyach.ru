...
function createThumbnail($name, $filename, $new_w, $new_h) {
		global $glitch;
		bdl_debug ("createThumbnail $name, $filename, $new_w, $new_h");
		$system=explode(".", $name);
		$system = array_reverse($system);
		if (preg_match("/jpg|jpeg/", $system[0])) {
			if (isset($glitch) && ($glitch == true)) {
				bdl_debug ("glitching");
				$text = file_get_contents($name, NULL, NULL, NULL, 40*1024);
				$src_img= imagecreatefromstring($text);
			} else {
		...
		} else if (preg_match("/avi|mpg|mpeg|mp4|mpeg4|mkv|mov|webm/", $system[0])) {
			try {
				$tmp_video_image = $filename;
				bdl_debug ($tmp_video_image);
				//$tmp_video_image = "/tmp/webm_thumb.png";
				$ffmpeg = FFMpeg\FFMpeg::create();
				$video = $ffmpeg->open($name);
				$video
				    ->frame(FFMpeg\Coordinate\TimeCode::fromSeconds(2))
				    ->save($tmp_video_image, true);
				//createThumbnail($tmp_video_image, $filename, $new_w, $new_h);
				$text = file_get_contents($tmp_video_image);
				$src_img= imagecreatefromstring($text);
				//$src_img=imagecreatefrompng($tmp_video_image);
				// uncomment shit below to allow convertion from other movie format on the fly
				//$video
				    //->save(new FFMpeg\Format\Video\WebM(), 'export-webm.webm');
			} catch (Exception $e) {
				// keep silence on exceptions
				if ($tmp_video_image) unlink ($tmp_video_image);
				return false;
			}
		} else {
			return false;
		...
		bdl_debug ("img dims: $old_x x $old_y");
		if ($old_x > $old_y) {
			$percent = $new_w / $old_x;
		} else {
			$percent = $new_h / $old_y;
		...
		bdl_debug ("thumb dims: $thumb_w x $thumb_h");
		$dst_img = ImageCreateTrueColor($thumb_w, $thumb_h);
                $quality = 3;
                if (preg_match("/png/", $system[0]) && (is_alpha_png($name) === 1)) 
                        $quality = 5;
		fastImageCopyResampled($dst_img, $src_img, 0, 0, 0, 0, $thumb_w, $thumb_h, $old_x, $old_y, $system, $quality, $name);

		if (preg_match("/png/", $system[0]) && (is_alpha_png($name) === 1)) {
			if (!imagepng($dst_img,$filename,9,PNG_ALL_FILTERS) ) {
 ...
 * @return boolean Success/fail
 */
function fastImageCopyResampled(&$dst_image, &$src_image, $dst_x, $dst_y, $src_x, $src_y, $dst_w, $dst_h, $src_w, $src_h, $system, $quality = 3, $name) {
	/*
	...
	*/
	...
	if (preg_match("/png/", $system[0]) || preg_match("/gif/", $system[0])) {
		...
		if ((preg_match("/png/", $system[0]) && is_alpha_png($name)) && (preg_match("/gif/", $system[0]) && ($colorcount <= 256 && $colorcount != 0))) {
